...
<GetStoreRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  <RequesterCredentials>
		...
		function EndItem($ebay_account,$itemid,$id){
    		...
    		<eBayAuthToken>'.$token.'</eBayAuthToken>
			 ...
			 $status = '<br><font color="#FF0000">'.$data['EndItemResponse']['Errors']['LongMessage'].'</font>';
                 if('The auction has already been closed.'==$data['EndItemResponse']['Errors']['LongMessage']){
                     $ss	 = "update ebay_list set status='1' where id='$id'";
                     $dbcon->execute($ss);
                     return '<br><font color="#00CC00">下架成功 The auction has already been closed</font>';
                 }
			 }else{
	...
	function ReListItem($ebay_account, $ItemID, $id){
			 $verb = 'RelistItem';
            $token		= geteBayaccountToken($ebay_account);
			 global $devID,$appID,$certID,$serverUrl,$siteID,$detailLevel,$compatabilityLevel,$dbcon,$user;
            $xmlRequest='<?xml version="1.0" encoding="utf-8"?>
<RelistItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <Item>
    <ItemID>'.$ItemID.'</ItemID>
			 ...
			 $ack = $data['RelistItemResponse']['Ack'];
			 
			 if($ack == 'Failure'){
						
			     $status = '<br><font color="#FF0000">'.$data['RelistItemResponse']['Errors']['LongMessage'].'</font>';
         
			 }else{
                 $newitem=$data['RelistItemResponse']['ItemID'];
                 $status = '<br><font color="#00CC00">relist success new itemID is:'.$newitem.'</font>';
                 getItemdetails($ebay_account,$newitem);
			 }
			 return $status;
...
print_r($data);
    ...
    <EntriesPerPage>200</EntriesPerPage>
			...
			if(count($ss) == 0){
				...
				$ss		  .= ",status,ebay_account,StartPrice,addtime) values('$BuyItNowPricecurrencyID','$BuyItNowPrice','$ItemID','$StartTime','$ViewItemURL','$ExpressListing','$ViewItemURLForNaturalSearch',";
				...
				$ss		  .= "'$TimeLeft','$Title','$QuantityAvailable','$SKU','$PictureDetailsGalleryURL','$status','$account','$StartPrice','$addtime')";
		...
		function GetMyeBaySelling($account,$token){
			...
			$totalitems		= $data['GetMyeBaySellingResponse']['ActiveList']['ItemArray']['Item'];
			 ...
			 for($i=0;$i<count($totalitems);$i++){
				...
				$Title					= mysql_real_escape_string($totalitems[$i]['Title']);
				$SKU					= $totalitems[$i]['SKU'];
				...
				if(count($ss) == 0 && $ListingType != 'Chinese' ){
						$addtime	=time();
                        $dataLocal	= getItem($token,$ItemID,'');
                        $Location   = $dataLocal[2];
						$ss		= "insert into ebay_list(status,ItemID,ViewItemURL,QuantitySold,Site,Quantity,Title,SKU,ListingType,StartPrice,ebay_account,ebay_user,QuantityAvailable,StartPricecurrencyID,GalleryURL,addtime,Location) value('0','$ItemID','$ViewItemURL','$QuantitySold','$Site','$Quantity','$Title','$SKU','$ListingType','$StartPrice','$account','$user','$QuantityAvailable','$StartPricecurrencyID','$GalleryURL','$addtime','$Location')";
...
echo $xmlRequest;
			 ...
			 for($i=0;$i<count($totalitems);$i++){
				...
				$Site					= $totalitems[$i]['Site'];
				$Location				= $totalitems[$i]['Location'];
				$StartPricecurrencyID	= $totalitems[$i]['StartPrice attr']['currencyID'];
				$StartPrice				= $totalitems[$i]['StartPrice'];
				//$Title					= $totalitems[$i]['Title'];
				$Title					= mysql_real_escape_string($totalitems[$i]['Title']);
				...
				if(count($ss) == 0 && $ListingStatus =='Active' && $ListingType != 'Chinese' ){
						...
						$addtime	=time();
						$ss		= "insert into ebay_list(status,ItemID,ViewItemURL,QuantitySold,Site,Quantity,Title,SKU,ListingType,StartPrice,ebay_account,ebay_user,QuantityAvailable,GalleryURL,addtime,Location) value('0','$ItemID','$ViewItemURL','$QuantitySold','$Site','$Quantity','$Title','$SKU','$ListingType','$StartPrice','$account','$user','$QuantityAvailable','$GalleryURL','$addtime','$Location')";
  ...
  <IncludeItemSpecifics>true</IncludeItemSpecifics>
   <DetailLevel>ReturnAll</DetailLevel>
		...
		if(count($ss) == 0){
			...
			$addtime	=time();	
			$ss		= "insert into ebay_list(PayPalEmailAddress,status,ConditionID,ItemID,Country,ebay_listingreturnmethodid,ReservePrice,PictureURL01,PictureURL02,PictureURL03,PictureURL04,ViewItemURL,DispatchTimeMax,BuyItNowPrice,Location,CategoryID,Description,HitCount,QuantitySold,BidCount,Site,Quantity,Title,SKU,ListingType,ListingDuration,TimeLeft,StartPrice,GalleryType,GalleryURL,ebay_account,ebay_user,addtime) value('$PayPalEmailAddress','$status','$ConditionID','$ItemID','$Country','$ebay_listingreturnmethodid','$ReservePrice','$PictureURL01','$PictureURL02','$PictureURL03','$PictureURL04','$ViewItemURL','$DispatchTimeMax','$BuyItNowPrice','$Location','$CategoryID','$Description','$HitCount','$QuantitySold','$BidCount','$Site','$qty','$Title','$SKU','$ListingType','$ListingDuration','$TimeLeft','$StartPrice','$GalleryType','$GalleryURL','$ebay_account','$user','$addtime')";
