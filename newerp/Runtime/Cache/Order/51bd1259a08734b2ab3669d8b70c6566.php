...
<body>
    ...
    <div class="ibox float-e-margins" style="margin:0 auto;height: 100%;padding:20px;width:100%;background:#fff;">
        ...
        <table class="table custom-table">
            ...
            <?php if(is_array($data)): foreach($data as $key=>$a): ?><tr class="detilspan <?php if($a['rightnow']) echo 'rightnow' ?>">
                    ...
                    <td style="text-align: left; ">
                        ...
                        <div class="skuArr" style="margin-top: 10px">
                            <?php if(is_array($a["skuArr"])): foreach($a["skuArr"] as $sk=>$sku): ?><div class="box">
                                <span><?php if($sk < 1): echo ($store[$a['ebay_warehouse']]); else: ?>&nbsp;<?php endif; ?></span>
                                <span style="width: 8%;cursor:pointer;color: blue" onclick="bigpic('<?php echo ($sku["goods_pic"]); ?>')">查看图片</span>
                                <span style="width: 10%"><?php echo ($sku["qty"]); ?>*<?php echo ($sku["sku"]); ?></span>
                                <span style="width: 6%"><?php echo ($sku["ispacking"]); ?></span>
                                <span class="<?php if($sku['issetW']): ?>issetW<?php endif; ?> weight" style="width: 8%;cursor:pointer;" data-weight="<?php echo ($sku["weight"]); ?>" onclick="updateWeight('<?php echo ($a["ebay_id"]); ?>','<?php echo ($sku["sku"]); ?>','<?php echo ($a["abnormal"]["abnormal"]); ?>','<?php echo ($a["abnormal"]["abnormal_id"]); ?>',this)"><?php echo ($sku["weight"]); ?>kg</span>
                                <span style="width: 5%;color: #0000cc;cursor:pointer;" onclick="lookLogs('<?php echo ($sku["sku"]); ?>')">日志</span>
                                <span style="width: 52%;margin-left: 1%"><?php echo ($sku["goods_name"]); ?></span>
                                </div>
                    ...
                    <td style="text-align: center;">
                        <?php echo ($a["baleuser"]["baleuser"]); ?>
                        <br>
                        <?php if(($a["baleuser"]["baletime"]) != ""): echo date('Y-m-d H:i:s',$a['baleuser']['baletime']); endif; ?>
                    </td>
                    <td style="text-align: center;" id="abnormal_<?php echo ($a["ebay_id"]); ?>">
