...
class BaseService {
	...
	public function __construct() {
		if ('development' == APP_ENV) {
			...
			$this->_TOKEN 	= C('TOKEN');
			$this->_postUrl = 'http://erptest.spocoo.com/t.php?s=/Apiwms/';
		}elseif ('test' == APP_ENV) {
            $this->_CLI_KEY = C('CLI_KEY');
            $this->_TOKEN 	= C('TOKEN');
        ...
        $this->currentid=C('CURRENT_STORE_ID');
	...
	protected function getErpData($requestData,$action) {
        ...
        if(I('debug')){
            echo "BaseService.class.php \n";
            print_r($ret);
            print_r([$requestData,$this->_postUrl.$action]);
        }
		//var_dump($ret);
