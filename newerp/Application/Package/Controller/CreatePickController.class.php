...
class CreatePickController extends CommonController
{
    ...
    public function index()
    {
        ...
        $resultArr = $ebayCarrierModel -> alias('a')
            ...
            -> where(['a.ebay_warehouse' => $currStoreId, 'a.name' => ['in', $this -> allowCarriers]])
            -> group('b.id')
        ...
        foreach ($resultArr as $key => $val) {
            $resultArr[$key]['sub'] = $ebayCarrierModel
                -> where(['ebay_warehouse' => $currStoreId, 'CompanyName' => $val['id'], 'name' => ['in', $this -> allowCarriers]])
