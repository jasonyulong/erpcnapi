...
class CheckService{
    ...
    function CheckOrder(){
        ...
        $PickCheckModel = new PickCheckModel('','',C('DB_CONFIG2'));
        ...
        $sql='truncate table pick_check;';
        $PickCheckModel->execute($sql);
        ...
        $map['b.pick_status']=['eq',2];
        $field="a.ebay_id,a.ebay_carrier";
        $Orders=$OrderModel->alias('a')->join('inner join erp_order_type b using(ebay_id)')->where($map)->field($field)->select();
        ...
        // 以上这些傻逼 都是 已经生成了 捡货单的，不知道是不是真的生成了。
        foreach($Orders as $List){
            $ebay_id=$List['ebay_id'];
            $ebay_carrier=$List['ebay_carrier'];
            $Rs=$PickOrderDetailModel->where(['ebay_id'=>$ebay_id,'is_delete'=>0])->find();
            if(!$Rs){ // 如果不存在的话 就是有问题！！！
                // 还是再查一遍 看看有没有同步进来
                $RR=$OrderTypeModel->where(['ebay_id'=>$ebay_id])->field('pick_status')->find();
                if($RR['pick_status']==1){
                    // 没救了 还是等于2
                    //恭喜你解放了  数据同步过来了
                    continue;
                ...
                echo $ebay_id."\n";
                $save                 = [];
                $save['ebay_carrier'] = $ebay_carrier;
                $save['ebay_id']      = $ebay_id;
                $PickCheckModel->add($save);
            }
        }
    }
    /**
    ...
    */
    ...
    //检查 是不是要删掉 捡货单
    private function PickOrderNeedDelete($ordersn){
        $PickOrderDetailModel = new PickOrderDetailModel('','',C('DB_CONFIG2'));
    ...
    function allocationInventoryBYConfirm($ordersn){
        $pick_order_confirm=new PickOrderConfirmModel('','',C('DB_CONFIG2'));
