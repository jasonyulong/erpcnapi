...
<table style="width:100%"><tr><td><div class='moduleTitle'>
    <h2><?php echo "扫描一次".$status.' '.$str;?> </h2>
                <div >
                     &nbsp;&nbsp;&nbsp;
                        ...
                        <table style="max-width:100%" border="0" align="center">
                            <tr>
                                <td width="100%" colspan="3"><DIV style="font-size:36px">1.通用跟踪号:
                                        ...
                                        <select id="curr" onchange="changeCarrier(this)">
                                            <?php
                            ...
                            <tr>
                                <td style="width: 40%;">
                                    <div style="font-size:36px">2.同步重量：&nbsp;
                                        <input name="currentweight" type="text" onkeyup="getnewweight(event)" id="currentweight" style="width:330px; height:50px; font-size:46px" />
                                        <span id="debug_prev_weight" style="color:#911;font-size:12px;"></span>
                                    </div>
                                    <div>
                                        <input type="hidden"   id="confirmOrder" checkWeight="0" WeightAbs="0" calcWeight="0" />
                                        <span style="font-size:36px">计算重量：</span>
                                        <span id="mstatus3" style="font-size:36px;color:#F11"></span>
                                    </div>
...
function checkByTrackNo(event){
    ...
    // hank 20180322 万欧物流的加拿大、澳大利亚 、意大利专线需要截取跟踪号，截取办法：E后面取10位数字
    if(curr == '意大利专线-万欧'){
        ebayid = ebayid.substring(0,11);
    }
    curr	=  encodeURIComponent(curr);
    var url		=  "getajax4.php?newcheckorder=1";
    var param	=  "nosleep=1&type=newcheckorder&ebayid="+ebayid+"&curr="+curr;
    $("#order").blur();
    $.ajax({
        type:"POST",
       // async: false,
        ...
        success:function(text){
            ...
            */
            ...
            if(text==-300){
                showMsg("核对失败:订单没有扫描/或者被拦截(查日志),请先扫描订单,然后等待5分钟后再同步重量!",false);
                play(false);
                return;
            }
...
function getnewweight(event) {//扫描第二枪
    ...
    if(CurrentOrderCheckStatus==0){
        play(false);
        showMsg("当前订单没有验证!",false); return;
    }

    if(CurrentOrderCheckStatus==-1){
        play(false);
        showMsg("当前订单验证失败，禁止同步重量!",false); return;
    }
