...
<table style="width:100%"><tr><td><div class='moduleTitle'>
    <h2><?php echo "订单扫描【仅同步重量】".$status.' '.$str;?> </h2>
                <div >
                    ...
                    <a href="t.php?s=Order/Scan">称重(2018)</a>
                </div>
</div>
                            ...
                            <tr>
                                <td width="100%" colspan="3"><DIV style="font-size:36px">1.通用跟踪号:
                                        ...
                                        <select name="curr" id="curr" onchange="changeCarrier(this)">
                                            <?php
                                            $sql = "SELECT name,weightmin,weightmax FROM ebay_carrier a WHERE ebay_warehouse = 196 GROUP BY name ORDER BY status ASC,`name`";
                                            ...
                                            for ($i = 0; $i < count($sql); $i++) {
                                                ...
                                                if ($shipping == $name) echo 'selected="selected"'; ?> weightMin="<?php echo  (int)$sql[$i]['weightmin']; ?>" weightMax="<?php echo  (int)$sql[$i]['weightmax']; ?>" ><?php echo $name; ?></option>
                                            <?php } ?>
                                        </select>
                                        <select id="openSound">
...
function checkByTrackNo(event){
    ...
    firstCurrentWeight = getWeight();
     //如果超重31kg，提醒装袋
    var totalCurrentWeightObj = $(".totalCurrentWeight").html();
    var totalCurrentWeight = parseInt($.trim(totalCurrentWeightObj));
    if(totalCurrentWeight/1000 > 31){
        showMsg("产品计算重量超重请装袋",false);
        play(false);
        return;
    }
    //清空隐藏域--重量范围
    ...
    $.ajax({
       ...
       // async: false,
        ...
        success:function(text){
            ...
            /**
            *测试人员谭 2018-04-10 20:11:56
            *说明:添加ebay_id主键用于查询
            */
            if(text.indexOf('_') > 0){
                var newText = text.split('_');
                text = newText[0];
                orderId = newText[1];
            }
...
function getnewweight() {//扫描第二枪
    ...
    if(currentweight <= 10){
        key = 0.4;
    }else if( (currentweight > 10) && (currentweight <= 20)){
        key = 0.3;
    }
    //20g允许误差15%
    //验证重量
    var begin = firstCurrentWeight - (firstCurrentWeight*key);
    var end = firstCurrentWeight + (firstCurrentWeight*key);
    if(currentweight>end || currentweight < begin) {
        //清空
        $("#order").select();
        $("#order").focus();
        clearPrevError();
        showMsg("重量不稳定请重试",false);
        return;
    }
...
function sendWeight(ebayid,currentweight){
    ...
    var packagingstaff	= $.trim(document.getElementById('packagingstaff').value);
    var curr	=  $.trim(document.getElementById('curr').value);
    if(curr.indexOf('PMS')==0){
        curr='PMS'
    }
    if (!curr) {
    ...
    var param	= "type=newupdateweight&ebayid="+ebayid+"&packagingstaff="+packagingstaff+"&currentweight="+currentweight+'&curr='+curr+'&orderid='+orderId;
    ...
    $.ajax({
       ...
       // async: false,
        ...
        success:function(text){
            ...
            if(text=='1'){
                showMsg("订单："+ebayid+"重量同步成功!",true); play(true);
