...
*/
    ...
    public function SaveWeightNew($trackNo,$currentweight,$curr,$number)
    {
        ...
        if($curr == '虚拟仓ubi加拿大特快专线')
        {
            $trackNo = substr($trackNo, 7, 16);
        }
        if($curr == 'UBI澳大利亚专线')
        {
            $length = strlen($trackNo);
            $first = substr($trackNo, 0, 18);
            if($first == '019931265099999891')
            {
                $trackNo = substr($trackNo, 18, $length - 18);
            }
        }
        //加个判断旧包裹是否处理完
